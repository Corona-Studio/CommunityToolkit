...
///     this attribute is
///     used, the generated property setter will also call <see cref="ObservableObject.OnPropertyChanged(string?)" /> (or
///     the equivalent
///     method in the target class) for the properties specified in the attribute data. This can be useful to keep the code
///     compact when
///     there are one or more dependent properties that should also be reported as updated when the value of the annotated
///     observable
///     property is changed. If this attribute is used in a field without <see cref="ObservablePropertyAttribute" />, it is
///     ignored.
///     <para>
///         In order to use this attribute, the containing type has to implement the <see cref="INotifyPropertyChanged" />
///         interface
///         and expose a method with the same signature as <see cref="ObservableObject.OnPropertyChanged(string?)" />. If
///         the containing
///         type also implements the <see cref="INotifyPropertyChanging" /> interface and exposes a method with the same
///         signature as
///         <see cref="ObservableObject.OnPropertyChanging(string?)" />, then this method will be invoked as well by the
///         property setter.
///     </para>
///     <para>
