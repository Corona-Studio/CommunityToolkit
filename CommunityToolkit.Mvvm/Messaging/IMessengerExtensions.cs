...
public static class IMessengerExtensions
{
    ...
    ///     Thrown if <paramref name="messenger" /> or <paramref name="recipient" />
...
#pragma warning disable IL2026
        ...
        {
            if (recipientType.Assembly.GetType("CommunityToolkit.Mvvm.Messaging.__Internals.__IMessengerExtensions") is
                    Type extensionsType &&
                extensionsType.GetMethod("CreateAllMessagesRegistrator", new[] { recipientType }) is MethodInfo
                    methodInfo)
            {
...
#pragma warning restore IL2026
    ...
    /// </remarks>
    /// <exception cref="System.ArgumentNullException">
    ///     Thrown if <paramref name="messenger" />, <paramref name="recipient" />
    ///     or <paramref name="token" /> are <see langword="null" />.
    /// </exception>
    [RequiresUnreferencedCode(
        "This method requires the generated CommunityToolkit.Mvvm.Messaging.__Internals.__IMessengerExtensions type not to be removed to use the fast path. " +
        "If this type is removed by the linker, or if the target recipient was created dynamically and was missed by the source generator, a slower fallback " +
...
#pragma warning disable IL2026
        ...
        static Action<IMessenger, object, TToken> LoadRegistrationMethodsForType(Type recipientType)
        {
            if (recipientType.Assembly.GetType("CommunityToolkit.Mvvm.Messaging.__Internals.__IMessengerExtensions") is
                    Type extensionsType &&
                extensionsType.GetMethod("CreateAllMessagesRegistratorWithToken", new[] { recipientType }) is MethodInfo
                    methodInfo)
            {
        ...
        static Action<IMessenger, object, TToken> LoadRegistrationMethodsForTypeFallback(Type recipientType)
        {
            ...
            BlockExpression body = Expression.Block(
                ...
                select Expression.Call(registrationMethod, new Expression[] { arg0, inst1, arg2 }));
...
#pragma warning restore IL2026
        ...
        Action<IMessenger, object, TToken> registrationAction =
            DiscoveredRecipients<TToken>.RegistrationMethods.GetValue(
                recipient.GetType(),
                static t => LoadRegistrationMethodsForType(t));
    ...
    ///     or <paramref name="token" /> are <see langword="null" />.
    /// </exception>
    public static void Register<TMessage, TToken>(this IMessenger messenger, IRecipient<TMessage> recipient,
        TToken token)
        where TMessage : class
        where TToken : IEquatable<TToken>
    {
    ...
    /// <exception cref="System.ArgumentNullException">
    ///     Thrown if <paramref name="messenger" />, <paramref name="recipient" />
    ///     or <paramref name="handler" /> are <see langword="null" />.
    /// </exception>
    public static void Register<TMessage>(this IMessenger messenger, object recipient,
    ...
    ///     or <paramref name="handler" /> are <see langword="null" />.
    /// </exception>
    public static void Register<TRecipient, TMessage>(this IMessenger messenger, TRecipient recipient,
    ...
    ///     or <paramref name="handler" /> are <see langword="null" />.
    /// </exception>
    public static void Register<TMessage, TToken>(this IMessenger messenger, object recipient, TToken token,
        MessageHandler<object, TMessage> handler)
        where TMessage : class
        where TToken : IEquatable<TToken>
    {
    ...
    /// <exception cref="System.ArgumentNullException">
    ///     Thrown if <paramref name="messenger" /> or <paramref name="message" />
    ///     are <see langword="null" />.
    /// </exception>
    ...
    /// </remarks>
    /// <exception cref="System.ArgumentNullException">
    ///     Thrown if <paramref name="messenger" /> or <paramref name="token" /> are
    ///     <see langword="null" />.
    /// </exception>
    ...
    {
        /// <summary>
        ///     The <see cref="MethodInfo" /> instance associated with
        ///     <see cref="Register{TMessage,TToken}(IMessenger,IRecipient{TMessage},TToken)" />.
        /// </summary>
        public static readonly MethodInfo RegisterIRecipient =
            new Action<IMessenger, IRecipient<object>, Unit>(Register).Method.GetGenericMethodDefinition();
    }
    ...
    {
        /// <summary>
        ///     The <see cref="ConditionalWeakTable{TKey,TValue}" /> instance used to track the preloaded registration action for
        ///     each recipient.
        /// </summary>
    ...
    /// <summary>
    ///     A class that acts as a static container to associate a <see cref="ConditionalWeakTable{TKey,TValue}" /> instance to
    ///     each
    ///     <typeparamref name="TToken" /> type in use. This is done because we can only use a single type as key, but we need
    ///     to track
    ///     associations of each recipient type also across different communication channels, each identified by a token.
    ///     Since the token is actually a compile-time parameter, we can use a wrapping class to let the runtime handle a
    ///     different
